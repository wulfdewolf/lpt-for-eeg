...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--freeze-encoder",
        action="store_true",
        help="Whether to keep the encoder stage frozen. "
        "Will only be done if not randomly initialized.",
    )
    parser.add_argument(
        "--random-init",
        action="store_true",
        help="Randomly initialized BENDR for comparison.",
    )
    parser.add_argument(
        ...
        for fold, (training, validation, test) in enumerate(
            ...
            if args.results_filename:
                if isinstance(test, Thinker):
                    ...
                    results.add_results_all_thinkers(
                        process, ds_name, test, Fold=fold + 1
                    )
