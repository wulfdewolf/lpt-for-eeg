...
# Per subject
for subject_id, subject_file in enumerate(os.listdir("data/competition/raw")):
    ...
    # Load raw file
    data = loadmat(
        "data/competition/raw/" + str(subject_file),
    ...
    for run_id, run in enumerate(data["data"][3:]):
        ...
        events = np.column_stack((run.trial, np.zeros(len(run.y), dtype=int), run.y))
